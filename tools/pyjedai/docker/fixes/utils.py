    ...
    def store_retained_ids(self, blocks: dict, data: Data) -> None:
        ...
        if (not self.subset):
            ...
            if (not data.is_dirty_er):
                self.d2_retained_ids = list(
                    range(data.num_of_entities_1, data.num_of_entities_1 + data.num_of_entities_2))
        else:
            ...
            if (data.is_dirty_er):
                ...
                for entity in blocks.keys():
                    if (self.from_source_dataset(entity, data)):
                        _d1_retained_ids_set.add(entity)
                    else:
                        _d2_retained_ids_set.add(entity)

                    neighbors = blocks[entity]
                    for neighbor in neighbors:
                        if (self.from_source_dataset(neighbor, data)):
                            _d1_retained_ids_set.add(entity)
                        else:
                            _d2_retained_ids_set.add(entity)
...
class EntityScheduler(ABC):
    ...
    def _get_average_weight(self) -> float:
        if (self._average_weight is None):
            self._average_weight = 0.0 if not self._get_neighbors_num() else (
                        float(self._get_total_weight()) / float(self._get_neighbors_num()))
...
class DatasetScheduler(ABC):
    ...
    def _sort_neighborhoods_by_avg_weight(self) -> None:
        ...
        self._sorted_entities: List = sorted(self._neighborhoods,
                                             key=lambda entity: self._neighborhoods[entity]._get_average_weight(),
                                             reverse=True)
...
class PredictionData(ABC):
    ...
    def get_name(self) -> str:
        ...
        if ('name' not in _matcher_info): raise ValueError(
            "Matcher doesn't have a name - Make sure its execution data has been calculated")
        return _matcher_info['name']

    def get_candidate_pairs(self) -> List[Tuple[float, int, int]]:
        if (self._candidate_pairs is None): raise ValueError(
            "Pairs not scheduled yet - Cannot retrieve candidate pairs")
        return self._candidate_pairs

    def get_duplicate_emitted(self) -> dict:
        if (self._duplicate_emitted is None): raise ValueError(
            "No information about the status of true positives' emission")
        return self._duplicate_emitted

    def get_total_emissions(self) -> int:
        _matcher_info: dict = self.get_matcher_info()
        if ('total_emissions' not in _matcher_info): raise ValueError(
            "Pairs not emitted yet - Total Emissions are undefined")
...
def get_class_function_arguments(class_reference, function_name: str) -> List[str]:
    ...
    if not inspect.isfunction(function_obj):
        raise ValueError(
            f"The provided name {function_name} does not correspond to a function in class '{class_reference.__name__}'.")
...
def values_given(configuration: dict, parameter: str) -> bool:
    ...
    """
    return (parameter in configuration) and (isinstance(configuration[parameter], list)) and (
                len(configuration[parameter]) > 0)
...
def necessary_dfs_supplied(configuration: dict) -> bool:
    ...
    return len(configuration['source_dataset_path']) == len(configuration['target_dataset_path']) == len(
        configuration['ground_truth_path'])
...
class FrequencyEvaluator(ABC):
    ...
    def save_distance_matrix(self) -> None:
        ...
        """
        distance_matrix_file_name = '_'.join(
            [self.indexing, self.dataset_identifier, self.vectorizer_name, self.tokenizer.split('_')[0], self.metric,
             "q" + str(self.qgram) + ".npy"])
    ...
    def retrieve_distance_matrix_file_path(self) -> Tuple[str, str]:
        ...
        _requested_indexing_file_name = '_'.join(
            [_requested_indexing, self.dataset_identifier, self.vectorizer_name, self.tokenizer.split('_')[0],
             self.metric, "q" + str(self.qgram) + ".npy"])
        _opposite_indexing_file_name = '_'.join(
            [_opposite_indexing, self.dataset_identifier, self.vectorizer_name, self.tokenizer.split('_')[0],
             self.metric, "q" + str(self.qgram) + ".npy"])
    ...
    def fit(self,
        ...
        if (d1_entities is None or d2_entities is None):
            raise NotImplementedError(
                f"{self.vectorizer_name} Frequency Evaluator Model - Dirty ER is not implemented yet")
        else:
